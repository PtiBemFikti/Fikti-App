    ...
    >
      <div className="container mx-auto md:flex md:justify-between md:items-center">
        <div className="hidden md:block">
          <Link href="#" className="flex items-center ml-5">
            <Image src="/pemira/logo-pemira.png" alt="logo pemira" width={40} height={40} />
          </Link>
        </div>
        <div className="hidden md:flex md:gap-10 space-x-4">
          <Link className="block text-lg text-white hover:text-purpleText duration-300 py-2" href="#">
            Home
          </Link>
          <Link className={`block ${pathname === '/nominasi' ? 'text-purpleText' : 'text-white text-lg'} hover:text-purpleText duration-300 py-2`} href="#candidates">
            Candidates
          </Link>
        ...
        <div className="hidden md:block">
          <Link href={'/'} className={`md:w-24 bg-gradient-to-l from-[#669BBC] to-[#003049] md:px-6 md:py-3 md:rounded-lg ${epilogue.className}`}>
      ...
      {isMenuOpen && (
        <motion.div className="flex flex-col gap-3 md:hidden mt-4" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.5, ease: 'easeInOut' }} onClick={toggleMenu}>
          <Link className="text-white text-lg hover:text-purpleText text-center" href="#">
            Home
          </Link>
          <Link className="text-white text-lg hover:text-purpleText text-center" href="#candidates">
            Candidates
          </Link>
          ...
          <div className="block md:hidden">
            <div className="flex justify-center items-center">
              <Link href={'/'} className={`bg-gradient-to-l from-[#669BBC] to-[#003049] text-sm px-6 py-3 rounded-lg ${epilogue.className}`}>
                {/* <Image src={'/pemira/user.svg'} alt="user" width={25} height={25}></Image>
            <ChevronDownIcon className={`h-6 w-12 transform ${isOpen ? '-rotate-180' : ''}`} /> */}
                Vote
              </Link>
