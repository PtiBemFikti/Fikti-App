...
const Navbar: React.FC = () => {
  ...
  return (
    <motion.nav initial={{ opacity: 1 }} animate={{ opacity: 1 }} transition={{ type: 'keyframes', duration: 0.5, delay: 0.5 }} className="bg-[#101415E5] shadow-[#00000080] border-[#1014151F] px-4 py-10 md:px-10 md:py-7 sticky top-0 z-50">
      <div className="container mx-auto md:flex md:justify-between md:items-center">
        <div className="hidden md:block">
          <Link href="#" className="flex items-center ml-5 gap-3">
            <Image src="/pemira/logo-universitas-gunadarma.png" alt="logo univ" width={40} height={40} />
            <Image src="/pemira/logo-kabinet.png" alt="logo fikti" width={40} height={40} />
            <Image src="/pemira/logo-fikti.png" alt="logo fikti" width={40} height={40} />
          </Link>
        </div>  
        <div className="hidden md:flex md:gap-10 space-x-4">
          <Link className="block text-lg text-white hover:text-purpleText duration-300 py-2" href="#home">
            Home
          </Link>
      ...
      <div className="flex justify-between">
        ...
        <div className="md:hidden">
          {/* Hamburger menu icon */}
          <button onClick={toggleMenu} className={`text-[#11111B] mr-4 p-1 ${isMenuOpen ? 'bg-[#25243A] backdrop-blur-sm shadow-md rounded-md ' : ''}`}>
      ...
      {isMenuOpen && (
        <motion.div className="flex flex-col gap-3 md:hidden mt-4" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.5, ease: 'easeInOut' }} onClick={toggleMenu}>
          <Link className="text-white text-lg hover:text-purpleText text-center" href="#about" id="AboutPage">
            Home
          </Link>
          ...
          <Link className="text-white text-lg hover:text-purpleText text-center" href="#faq">
            FAQ
          </Link>
